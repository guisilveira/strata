...
| AI agent command | The folder and item context menus read the current command when the menu opens, and hide the agent action while it is empty. The embedded terminal reads it again at launch, so changing it does not disturb a running agent. Selected paths replace a `{}` placeholder in the command, one argument per path for a bare `{}` and a single argument when `{}` sits inside a word; without a placeholder they follow the command. Paths always reach the agent as process arguments; no shell parses them. |
